...
def user_body(page: ft.Page, nav) -> ft.Control:
    ...
    def show_scan() -> None:
        body.controls = [_scan_view(on_scan_good, on_scan_low_quality)]
        page.update()
    ...
    async def on_scan_good(_e) -> None:
        await _run_scan(_MOCK_OK)

    async def on_scan_low_quality(_e) -> None:
...
def _scan_view(on_scan, on_scan_low_quality) -> ft.Control:
    ...
    dev_low_quality = ft.TextButton(
        "Test: düşük kalite sonucu",
        on_click=on_scan_low_quality,
    )
    ...
    return ft.Column(
        ...
        controls=[
            viewfinder,
            scan_button,
            ft.Row([dev_low_quality], alignment=ft.MainAxisAlignment.CENTER),
            section_card(
                "Son okumalar",
